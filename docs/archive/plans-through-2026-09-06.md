# 历史任务计划（归档至 2026-09-06）

此文件保留历史任务与当时的验证结论；当前执行状态以根目录 PLANS.md 为准。“已完成”不表示后来发现的问题也已修复。

# 计算审计缺陷修复执行（2026-08-14）

分支：`codex/fix-calculation-audit`。目标：参照 `docs/calculation-audit-repair-spec.md` 完成 13 项计算缺陷修复，严格执行测试先行、最小生产代码修改与阶段门禁。

## 执行计划

| 阶段 / 任务 | 状态 | 范围与说明 |
|---|---|---|
| 01 Arudha Pada | 已完成 | 废除 `pada + 10`；实现本宫 `(H+9)%12` 与对宫 `(H+3)%12`；旧代码在白羊/双鱼例外下分别输出 10/9 而失败，修复后 5 项聚焦测试通过 |
| 02 D30 Trimsamsha | 已完成 | 修正偶数星座分段为 5(Taurus), 7(Virgo), 8(Pisces), 5(Capricorn), 5(Scorpio) 并修正跨度映射；旧代码在偶数分段边界均产生跨座错误，修复后 16 项聚焦测试全过 |
| 03 8星 Chara Karaka | 已完成 | 定义 7 星制与 8 星制（含 Pitri Karaka）独立名称表，支持完整 8 项输出，增加 `requested_system`、`complete`、`missing_planets` 完整性元数据与缺星测试 |
| 04 Yogakaraka | 已完成 | 锁定 12 上升星座 Yogakaraka 映射；旧代码在巨蟹(Venus)、狮子(Mercury)、天秤(None)、水瓶(None)上失败，修复后 13 项测试全过 |
| 05 Ekadhipatya Shodhana | 已完成 | 从 trikona 深复制，按 7 曜入驻实现 7 条完整规则矩阵与 _reduce_ekadhipatya_pair helper；旧代码未引入 helper 且复制 rekha 失败，修复后 12 项测试全过 |
| 06 Nathonatha Bala | 已完成 | 引入 LMT 经度换算 `jd_lmt = jd_ut + lon/360` 并移除未使用的 `call_houses_ex`；旧代码忽略经度导致 120°E/120°W 与 0° 输出无差异而失败，修复后 6 项测试及 240 项 Jyotish 阶段门禁全过 |
| 07 Zodiacal Releasing | 已完成 | 基于 Classical Hybrid Profile (L1=365.2425d, L2=30d, L3=2.5d, L4=5h) 实现 `_zr_sub_periods_sequence`，支持子期累积、末端截断与第 13 期真实 LoB 对宫跳跃；5 项新测试及 112 项 Classical 阶段门禁全过 |
| 08 行星会合步长 | 已完成 | 提取生产 helper `_synodic_search_step` 并将步长从 `max` 修复为 `min`；增加 helper 测试与 spy_find_roots 生产调用参数捕获断言，7 项测试全过 |

| 09 组合盘象限宫位 | 已完成 | 修正象限宫位重建旋转基准为 `raw_angles['MC']` 与中点时间 `comp_jd`；消除 A/B 交换不对称性与 MC 偏转，25 项测试全过 |
| 10 跨 0° 星盘形态 | 已完成 | 基于环形最大缺口 `span = 360.0 - max_gap` 修复跨 0° 计算，严格保留既有独立分类阈值（Bundle <= 90°，Bowl < 180°，Locomotive 60°..150°，Bundle 与 Bowl 可共存）；31 项格局测试全过 |
| 11 Heliacal previous | 已完成 | 实现从历史起点向前递进的 `_find_previous_heliacal_event`，确保返回严格小于目标时刻的最邻近前一次事件，异常上报 warnings；通过 Moon `EVENING_FIRST` 短周期精确验证 |
| 12 Alcocoden 排名 | 已完成 | 修正 Alcocoden 候选排名语义：初始值与不合格候选统一置 `rank: None`，仅对 `selectable_pool` 赋予 `rank = 1..N`；旧代码对淘汰候选赋予 1..5 整数导致语义混淆失败，修复后 85 项古典测试全过 |
| 13 JD 午夜进位 | 已完成 | 重构 `_utc_from_jd` 使用 `datetime + timedelta(microseconds=...)`，避免微秒溢出/模 24 导致的跨日/跨月倒退回退 bug；8 项测试全过 |
| 14 完整门禁与收口 | 已完成 | 1086+ 项 pytest、223 项 Swift 测试（30 个 suite）、36 项后端 smoke 全部通过；`git diff --check` 0 警告 |

### ZR_UNIT_PROFILE 规范定义 (Classical Hybrid Profile)
- **L1 (年)**: 1 个周期数 = `ZR_PERIOD_YEARS[sign]` 年，折算天数 `years * 365.2425`（通过 `add_years_approx` 采用回归年）。
- **L2 (月)**: 1 个单位 = 30.0 天（固定月单位），每个星座子期天数 = `ZR_PERIOD_YEARS[sign] * 30.0` 天。
- **L3 (日)**: 1 个单位 = 2.5 天（30/12 固定日单位），每个星座子期天数 = `ZR_PERIOD_YEARS[sign] * 2.5` 天。
- **L4 (分期/时)**: 1 个单位 = 2.5/12 天 = 5 小时（5/24 天），每个星座子期天数 = `ZR_PERIOD_YEARS[sign] * (5.0 / 24.0)` 天。
- **子期截断**: 各层子期由父期起始累加，超过父期 `end_local` 时强制截断到父期 `end_local`。
- **LoB 定义**: 子期走完第一轮 12 星座后（step=12），下一期跳跃至该层所属父期星座的对宫 `(origin_sign + 6) % 12`，之后由此对宫按黄道顺行继续直到父期结束。
- **激活判定**: `cursor <= reference_dt < sub_end`（半开区间）。

## 边界

- 严格遵循 `docs/calculation-audit-repair-spec.md` 规定的 13 项任务与禁止事项。
- 绝不改动 Horary v2.1、KP、Rectifier 或其他非审计算法。
- 不引入无关 UI 调整或 schema 破坏。

---

# 计算审计缺陷强约束修复规范（2026-08-14）

目标：新增 `docs/calculation-audit-repair-spec.md`，把计算审计中可复核的 13 项问题转换为低歧义、测试先行、逐项验收的实现规范；本任务只写修复规范，不修改计算代码、测试、fixture、版本或安装包。

## 执行计划

| 阶段 | 状态 | 范围 |
|---|---|---|
| 01 缺陷与源码契约复核 | 已完成 | 逐项核对当前实现、最小复现、现有测试盲区及项目内 Maitreya 参考实现 |
| 02 强约束修复规范编写 | 已完成 | 为 13 项缺陷规定修改范围、必须新增的失败测试、实现规则、禁止做法与验收条件 |
| 03 文档自检与任务记录 | 已完成 | 已检查 13 项任务标题、全部明确引用路径、执行命令、依赖门禁、完成条件与 `git diff --check`；CHANGELOG 已更新 |

## 边界

- 本任务不实施任何计算修复，不改 schema/fixture，不构建或打包。
- 保留工作区中已有的架构方案、评审、`PLANS.md` 与 `CHANGELOG.md` 改动；只追加本任务记录。
- 修复规范必须让后续执行者按任务顺序操作，禁止把 13 项混成一次无测试的大改。

---

# 架构重设计方案独立评审（2026-08-13）

分支：当前实际位于 `codex/feature-kp-horary` 的 dirty 工作区（本任务只做文档评审，正式提交前必须隔离）。目标：对 `docs/architecture-redesign.md` 做前端、后端、契约、测试与迁移路线的源码对照评估，将可执行结论独立落到 `docs/architecture-redesign-review.md`；不修改原方案、不实施重构。

## 执行计划

| 阶段 | 状态 | 范围 |
|---|---|---|
| 01 方案与仓库边界核验 | 已完成 | 完整读取方案稿、AGENTS、当前分支状态、现有 Swift/Python 调用链与项目契约 |
| 02 并行专项审计 | 已完成 | 后端 daemon/缓存/取消；前端 reducer/effect/共享状态；schema/fixture/导出/路线图三路只读核验 |
| 03 跨层结论汇总 | 已完成 | 判定为 Conditional Go；识别计算隔离、取消模型、缓存、默认 envelope、Effect 任务身份等阻断项 |
| 04 独立评审文档 | 已完成 | 新增 `docs/architecture-redesign-review.md`，记录证据、严重度、修订架构、阶段 -1 至 5 与开工门禁 |

## 边界

- 本任务只写评审文档及相应计划/变更记录，不修改 `docs/architecture-redesign.md` 或任何产品代码。
- 未运行构建、测试或打包；评审证据来自源码、测试、文档、Git 状态与只读诊断。
- 方案稿和评审稿当前仍位于混有 KP 未提交改动的工作区；正式提交前应隔离成独立文档提交或分支。

---

# 前后端架构重设计方案（2026-08-13）

分支：无（仅文档）。目标：基于现有 35+ 计算模式与理想使用方式，产出完整的前后端重设计方案（macOS 单机、本地常驻 daemon、TCA 风格状态机、渐进迁移路线图），落地到 `docs/architecture-redesign.md`，本次不写实现代码。

## 执行计划

| 阶段 | 状态 | 范围 |
|---|---|---|
| 01 现状量化审计 | 已完成 | 代码规模、依赖图、坏味道（149 @State、三件套模板、双进程管理、horary v2 循环 import、冷启动进程模型）均已量化确认 |
| 02 设计决策确认 | 已完成 | 与用户确认：macOS 单机为主 / 本地常驻服务 / TCA-Reducer 状态机 / 只出方案暂不落地 |
| 03 方案文档撰写 | 已完成 | `docs/architecture-redesign.md`：daemon 生命周期与协议、契约 envelope 化、mini-TCA 内核、状态域划分、导出统一、测试策略、阶段 0–5 迁移路线 |

## 边界

- 纯设计交付：不改产品代码、不跑构建、不落地任何重构。
- 与既有文档衔接：`docs/schemas/`（契约雏形）、`docs/frontend-refactor/05-viewmodel-extraction.md`（ViewModel 抽取先例）、`docs/backend-contracts.md` 是方案起点而非替代品。
- 方案中的「迁移路线图」仅作后续实现任务的输入，本任务不开启任何迁移分支。
...
| 03 提交与任务分支推送 | 已完成 | `e4f3d07` 已提交并推送 `codex/feature-kp-horary`；账号 `1223a80` 的 keyring 凭证已在沙箱外确认有效，任务分支与远端为 `0/0` |
| 04 合并 main 与远端核验 | 已完成 | `main` 已从 `27f5239` 快进到 `e4f3d07` 并推送 `origin/main`；首次推送后本地/远端为 `0/0`，架构设计稿保持在发布范围之外 |
...
| 06 CI runtime warning cleanup | completed | Official checkout/setup-python v7 releases and Node 24 declarations verified; upgraded workflow passed remote Swift/Python/backend smoke CI with no Node 20 deprecation annotation |
...
| 05 打包覆盖与产物验证 | 已完成 | `1.4.6 (48)` 覆盖 `/Applications`；签名、arm64、构建/安装全目录与二进制 SHA-256 一致、包内无 pyc；安装包 Horary smoke 通过 schema 且与 fixture 完全一致 |
| 06 清理与最终同步检查 | 已完成 | 删除 `.build`、`dist`、`.venv`、Python/pytest 与临时打包缓存约 659 MB；完整差异、提交边界与 Git 状态复核完成 |

完成标准：无未解决审查发现；全部必需验证通过；已安装应用与本次主线源码/版本一致；缓存已清理；最终 Git 状态和提交记录清楚可核查。

---

# 后端计算缺陷并行扫描与报告落盘（2026-08-26）

分支：`codex/fix-calculation-audit`（只读任务，不产生源码改动）。目标：7 路子代理按模块并行扫描后端计算缺陷，modern_timing.py 因子代理会话连续空返回由主代理人工逐行补审，全部发现落盘 `docs/backend-calculation-bug-scan-20260826.md`（含位置、现状/应然逻辑、判定依据、最小验证方法）。

| 阶段 | 状态 | 范围 |
|---|---|---|
| 01 模块拆分与并行扫描 | 已完成 | 7 路：核心/API/星历、古典、吠陀/KP、现代推运、关系盘/地图、卜卦/矫正/方向、扫描/能见度/形态 |
| 02 补审与汇总 | 已完成 | modern_timing.py 人工审计补位；去重分级：1 P1 / 11 P2 / 24 P3；近期 13 项修复复核成立 |
| 03 报告落盘 | 已完成 | `docs/backend-calculation-bug-scan-20260826.md` + CHANGELOG 条目 |

边界：纯只读扫描，未修改任何源码；PLANS/CHANGELOG 为唯一写入。后续修复按文档第五节分批执行。

---

# 源码导向项目熟悉（2026-09-06）

状态：已完成。范围：直接追踪当前工作树的 Swift 入口、状态、计算调用、Python 路由与计算底座、AI、存储、导出和测试；不以功能文档作为实现证据。已有改动归属后端计算修复及对应测试、历史任务记录，另有未跟踪审计/架构文档，本任务不修改或提交这些内容。

- [x] 阅读入口与主要调用链，核实功能边界。
- [x] 阅读跨语言、AI、存储、导出与测试实现；执行轻量验证。
- [x] 汇总源码证据、当前工作树状态与理解边界。

本任务不修改业务源码、不打包；仅追加本计划记录。

验证记录：67 个后端 Python 文件 AST 解析通过；使用现有 Framework Python 执行 constants/modern_points 共 16 项测试通过；时间点、古典、Horary 三种真实子进程请求成功返回 JSON，Horary schema 为 2.1。时间点样例因缺少 433 小行星星历跳过该点并给出警告。未执行 Swift 构建、UI 操作或全量计算准确性审计。终端默认 Python 缺 pytest，已复用现有环境，无安装和新增缓存。
