...
> 状态：计划修订完成。2026-09-24 执行进度：W01、W03 及 W04 中的 Mercury Hayz、Hellenistic 相位适配已修复并通过集成门禁；W02 已证明为生产误报。其余来源项仍按本计划的证据与决策条件推进。原版 [bugfix-master-plan-20260922.md](bugfix-master-plan-20260922.md) 保留；修法、分组与执行条件以本版为准，原始报告继续作为线索与历史证据。
>
...
2026-09-22 的计划编写轮只新建计划；其后的修复按具体工作包实施并记录状态。本文规划的实施范围为两份后端计算审查报告及原计划；前端独立审查、架构迁移、功能新增不纳入。不能把本文中列举的候选方案理解为已经选定。
...
登记表中的未完成阶段是**下一步动作**；已完成的条目以“已修复”或“已证明非缺陷”明确标注：
...
| 已修复（集成门禁通过） | 实现、失败测试与消费者核验已完成 | 后续回归仍由项目门禁持续检查 |
...
|---|---|---|---|---|---|
| R-P1-1 | Egyptian Aries / classical_dignity | A-1 | 已修复（集成门禁通过） | W01 | 独立原典确认 6/12/20/25/30；逐边界、尊贵/almuten/沿界数值链及当前真实输出夹具已验证 |
...
| R-P2-b | Mercury Hayz 中文前缀 / classical_dignity::hayz_status | B-2 | 已修复（集成门禁通过） | W04 | EV-07；复用 sect_agreement，昼夜与东方/西方 16 格正反例通过 |
...
| R-P2-g | rate=0 被吞 / method_families | B-3 | 已修复（集成门禁通过） | W03 | 0、缺失、bool、NaN/Inf、非法字符串与计算溢出；入口结构化错误 |
| R-P2-h | 字符串 false 变 True / method_families | B-3 | 已修复（集成门禁通过） | W03 | 真/假 JSON bool 分支，字符串拒绝；test_method_families |
...
| S-P2-2 | Hellenistic 相位 ID/状态失配 / hellenistic_audit、classical | C-7 | 已修复（集成门禁通过） | W04 | 复用上游稳定 ID，入/离逐行匹配并保留 False/零 orb；真实入口烟测通过 |
...
1. W01 已按独立来源完成 Egyptian Aries 修复与下游验证；W02 已由 EV-04/EV-13 证明为生产误报并关闭。两条来源记录仍分别归属 W01、W02，保持 87 条来源 ID 与 18 个工作包的追溯关系。
2. W03 已完成输入校验；W04 中 Mercury Hayz 与 Hellenistic 相位适配已完成，其余 W04 子项仍按各自证据门槛推进。W05 中确定的恒真条件/去重项、W09 键名、W16 已有错误协议缺口可进入小范围修复。
