...
- API 数字校验现可安全拒绝巨大整数；method_families 按请求的出生与参考时刻检查自定义 rate 所得弧度是否仍为有限值，溢出时沿现有 `invalid` 协议返回错误。
- 沿界 Swift 契约测试加入 Egyptian ASC 当前界终点 20° 的断言，以便旧历史夹具先暴露失败，再由当前后端真实输出重建夹具。
- 旧夹具在新断言下显示 21° 并失败；现已按 `docs/validation.md` 的命令用 `sample-distributions-pd-request.json` 的真实后端响应完整重建沿界夹具，保留当前 `periods` 与兼容 `boundaries` 字段。
- 更新 v2 来源登记与执行顺序，准确标出本轮已完成的 W01、W03、Mercury Hayz 和 Hellenistic 相位项；其他 W04 子项维持待处理状态。
- 合并后的完整门禁通过：Python 1149 项、Swift 227 项 / 31 组，包含 43 个实时后端解码样例；真实沿界夹具与当前后端输出逐字节一致。清理本轮 SwiftPM 临时构建目录及项目缓存。
...
- 根据 GitHub 官方 2026-07-20 稳定发布与 `action.yml` 的 Node 24 声明，将 CI 的 `actions/checkout`、`actions/setup-python` 同步升级到 v7；远端 Swift / Python / backend smoke 全绿且不再产生 Node 20 弃用告警。
