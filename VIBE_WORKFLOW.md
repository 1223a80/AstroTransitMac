# Vibe Coding 工作流指南（给完全不懂编程的我）

**核心事实**：这个项目从第一行代码开始，就是我用自然语言 + AI agent（主要是 Grok / Claude / Cursor 等）“vibe coding” 出来的。我完全不写代码、不懂 Swift、不懂 Python、不懂架构。

这不是缺点，这是项目的**真实开发模式**。我们要接受这个现实，并建立**专门保护这种模式的规则**，让项目不会因为“vibe 太猛”而变成无法维护的屎山。
